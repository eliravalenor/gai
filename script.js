...
async function getMentionedAIReply(postData, mentioningComment, mentionedContact) {
    ...
    const data = await window.apiService.callAPI(apiSettings.provider || 'openai_compat', apiSettings.url, apiSettings.key, apiSettings.model,
...
function showApiSettingsModal() {
    ...
    document.getElementById('apiProvider').value = apiSettings.provider || 'openai_compat';
...
async function testApiConnection() {
    const provider = document.getElementById('apiProvider').value || 'openai_compat';
    ...
    try {
        const data = await window.apiService.testConnection(url, key, provider);
...
async function saveApiSettings(event) {
    event.preventDefault();
    apiSettings.provider = document.getElementById('apiProvider').value || 'openai_compat';
